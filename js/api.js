...
async function apiFetch(endpoint, options = {}) {
    ...
    try {
        ...
        return { success: false, message: 'Network error – check XAMPP is running.' };
    }
}

const AuthAPI = {
    // ✅ role is now passed so the server can validate it alongside credentials
    login:    (email, password, role) => apiFetch('/auth.php?action=login', { method:'POST', body: JSON.stringify({ email, password, role }) }),
...
const SettingsAPI = {
    get:  (key) => apiFetch(`/settings.php?action=get&key=${encodeURIComponent(key)}`),
    set:  (key, value) => apiFetch('/settings.php?action=set', { method:'POST', body: JSON.stringify({ key, value }) }),
};