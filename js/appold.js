...
const App = {
    ...
    async init() {
        ...
        // Poll for new groups every 10 seconds (catches when admin adds you to a group)
        setInterval(() => this.pollNewGroups(), 10000);
    ...
    async loadGroups() {
        ...
        // Track group IDs so we can detect when new ones are added
        this._knownGroupIds = res.data.map(g => g.id);
        if (res.data.length > 0) await this.selectGroup(res.data[0].id, res.data[0].name);
        // Show settings btn for admin
    ...
    async loadArticles() {
        ...
        // Pinned articles first, then rest in chronological order (oldest first, newest at bottom)
       /* const pinned   = res.data.filter(a => a.is_pinned == 1);
        const unpinned = res.data.filter(a => a.is_pinned != 1);
        [...pinned, ...unpinned].forEach(article => feed.appendChild(this.buildArticleCard(article)));
     */
        const pinned   = res.data.filter(a => a.is_pinned == 1)
  ...
  async loadMessages() {
    ...
    // Play sound only when a genuinely new message arrives (not on first load)
    ...
    async pollNewGroups() {
        ...
        try {
            ...
            const currentIds = res.data.map(g => g.id);

            // Check if any new group was added
            const newGroups = res.data.filter(g => !this._knownGroupIds.includes(g.id));

            if (newGroups.length > 0) {
                // New group found - reload sidebar
                this._knownGroupIds = currentIds;
                await this.reloadGroupsSilently(res.data);

                // Notify user
                newGroups.forEach(g => {
                    this.playNotificationSound();
                    this.showToast(`You were added to "${g.name}"!`, 'ti-users');
                });
            }

            // Also check if a group was removed (deleted or removed from)
            const removedGroups = this._knownGroupIds.filter(id => !currentIds.includes(id));
            if (removedGroups.length > 0) {
                this._knownGroupIds = currentIds;
                await this.reloadGroupsSilently(res.data);
                // If current group was removed, switch to first available
                if (removedGroups.includes(this.currentGroupId)) {
                    if (res.data.length > 0) {
                        await this.selectGroup(res.data[0].id, res.data[0].name);
                    } else {
                        this.currentGroupId = null;
                        this.currentGroupName = null;
                    }
                    this.showToast('You were removed from a group', 'ti-info-circle');
                }
            }
        } catch(e) {}
    },

    // Reload group list in sidebar without changing current selection
    async reloadGroupsSilently(groupData) {
        const list = document.getElementById('group-list');
        if (!list) return;
        list.innerHTML = '';
        const colors = [
        ...
        groupData.forEach((group, i) => {
            ...
            const initials = group.name.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2);
            const el = document.createElement('div');
            el.className = 'group-item' + (group.id === this.currentGroupId ? ' active' : '');
            el.dataset.groupId = group.id;
            el.innerHTML = `
                <div class="group-avatar" style="background:${c.bg};color:${c.fg}">${initials}</div>
                <div class="group-info">
        ...
        try {
            ...
            if (this._lastMessageId > 0 && latestId > this._lastMessageId) {
                ...
                if (latestMsg.user_id != me?.id) {
                    ...
                    // Reload messages if chat tab is active
    ...
    async confirmAddMember() {
        ...
        if (res?.success) {
            ...
            const groupEl = document.querySelector(`.group-item[data-group-id="${this.currentGroupId}"] .group-sub`);
            if (groupEl) {
                const count = this._currentMembers.length;
                groupEl.textContent = `${count} member${count !== 1 ? 's' : ''}`;
            }
        } else {
...
// code end
    ...
    async loadNotifications() {
        ...
        res.data.forEach(n => {
            ...
            el.addEventListener('click', async () => {
                ...
                this.loadNotificationBadge();
                // If it's a group invite, reload groups list
                if (n.notification_type === 'group_invite') {
                    await this.loadGroups();
                    this.showToast('Group list refreshed!', 'ti-users');
                }
            });
            feed.appendChild(el);
        });
    ...
    filterArticles(q) {
        ...
        cards.forEach(card => {
            ...
            const auth  = card.querySelector('.article-author')?.textContent?.toLowerCase() || '';
    ...
    async openManageGroupModal() {
        ...
        // Parse description (might be JSON with invite token)
        let desc = group.description || '';
        try {
            const parsed = JSON.parse(desc);
            if (parsed.invite_token) desc = ''; // don't show internal metadata
        } catch(e) {}
        ...
        // Messages count - fetch separately
    ...
    async saveGroupSettings(e) {
        ...
        if (res?.success) {
            ...
            await this.loadGroups(); // refresh sidebar
        } else {
    ...
    async openInviteLinkModal() {
        ...
        if (res?.success) {
            ...
            if (urlEl) urlEl.textContent = 'Failed – check admin permissions';
        }
    },
    ...
    async regenerateInviteLink() {
        ...
        const res = await GroupsAPI.inviteLink(this.currentGroupId);
        if (res?.success) { if (urlEl) urlEl.textContent = res.data.link; this.showToast('New link generated!', 'ti-check'); }
    },

    shareInviteWhatsApp() {
        const url = document.getElementById('invite-link-url')?.textContent;
        if (!url || url === 'Generating…') return;
        window.open('https://wa.me/?text=' + encodeURIComponent(`Join "${this.currentGroupName}" on ArticleHub: ${url}`), '_blank');
    },
...
${caption}`;
    ...
    initOnlineStatus() {
        ...
        window.addEventListener('beforeunload', () => {
            // Best-effort: ping server to mark offline
            if (Auth.isLoggedIn()) {
    ...
    bindTabBar() {
        // Set initial state - Articles tab active, message input hidden
