...
function updateGroup(): void {
    ...
    if (!$groupId || !$name) {
        jsonError('Group ID and name required');
    }

    getDB()->prepare("
        UPDATE groups 
        SET name = ?, description = ?, invite_token = ?
        WHERE id = ?
    ")->execute([
        $name,
        $description,
...
function inviteLink(): void {
    $user    = requireAdmin();
    $groupId = (int)($_GET['id'] ?? 0);
    if (!$groupId) {
        jsonError('Group ID required');
    }
    $db    = getDB();
    $token = bin2hex(random_bytes(16));
    // Store token in separate column
    $db->prepare("UPDATE groups SET invite_token = ? WHERE id = ? ")->execute([$token,$groupId]);
    // Build invite link
    $baseUrl = rtrim($_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'], '/');
    $link = $baseUrl . '/article-dashboard/join.php?token=' . $token;
    jsonSuccess(['link' => $link ]);
}
...
function addMember(): void {
    ...
    if (!$groupId || !$userId) {
        jsonError('Group ID and User ID required');
    }
    ...
    createNotification(
        ...
        'group',
        $groupId,
        $user['name'] . ' added you to the group "' . $groupName . '"'
    );
